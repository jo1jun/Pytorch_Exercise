...
original_train = datasets.FashionMNIST('./.data',
                   train=True,
                   download=True,
                   transform=transforms.Compose([
                       transforms.ToTensor(),                       # 교재에는 학습 데이터가 2배가 되었다고 하는데 기존 데이터에 추가되지 않고 데이터수 그대로이다.
                       transforms.Normalize((0.1307,), (0.3081,))   # 평균과 표준편차로 normalize
                   ]))

flip_train = datasets.FashionMNIST('./.data',
                   train=True,
                   transform=transforms.Compose([
                       transforms.RandomHorizontalFlip(1),           # 랜덤하게 왼쪽 오른쪽 뒤집기 (default 로 p = 0.5 즉, 1/2 확률로 뒤집기.)
                       transforms.ToTensor(),                       # 교재에는 학습 데이터가 2배가 되었다고 하는데 기존 데이터에 추가되지 않고 데이터수 그대로이다.
                       transforms.Normalize((0.1307,), (0.3081,))   # 평균과 표준편차로 normalize
                   ]))

datasets_train = torch.utils.data.ConcatDataset([original_train, flip_train])

original_test = datasets.FashionMNIST('./.data',
                   train=False,
                   transform=transforms.Compose([
                       transforms.ToTensor(),                       # 교재에는 학습 데이터가 2배가 되었다고 하는데 기존 데이터에 추가되지 않고 데이터수 그대로이다.
                       transforms.Normalize((0.1307,), (0.3081,))   # 평균과 표준편차로 normalize
